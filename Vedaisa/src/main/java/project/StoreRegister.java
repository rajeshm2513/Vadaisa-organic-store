...
@MultipartConfig @WebServlet("/StoreRegister") 
public class StoreRegister extends HttpServlet { 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException { 
		request.setCharacterEncoding("UTF-8"); // ✅ Read inputs
		...
		String address = getFieldValue(request.getPart("address")); // ✅ Image upload 
		...
		try { 
			...
			Connection conn = DriverManager.getConnection( "jdbc:oracle:thin:@localhost:1521:xe", "system", "rajesh" ); // ✅ Duplicate Validation 
			PreparedStatement checkDuplicate = conn.prepareStatement( "SELECT COUNT(*) FROM store_registration WHERE store_username=? OR email=? OR phone=? OR store_name=?" ); 
			checkDuplicate.setString(1, storeUsername); checkDuplicate.setString(2, email); checkDuplicate.setString(3, phone); 
			...
			{ conn.close(); response.sendRedirect("storeregister.jsp?status=duplicate");
			return; }
			// ✅ Insert record
			String sql = "INSERT INTO store_registration (store_id, store_username, full_name, store_name, email, phone, password, certification_url, city, address, image_url) VALUES (store_reg_seq.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, storeUsername); ps.setString(2, fullName); 
		...
		} else { response.getWriter().println("Database Error: " + ex.getMessage()); } 
